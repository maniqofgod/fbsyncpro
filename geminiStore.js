...
class GeminiStore {
    ...
    async addApi(apiKey, name = null, id = null) {
        try {
            const data = await fs.readFile(DB_PATH, 'utf8');
            const db = JSON.parse(data);
            ...
            if (id) {
                // Update existing API
                const apiIndex = db.geminiApis.findIndex(api => api.id === id);
                if (apiIndex >= 0) {
                    if (apiKey && apiKey !== 'existing') {
                        db.geminiApis[apiIndex].apiKey = apiKey;
                    }
                    db.geminiApis[apiIndex].name = name || db.geminiApis[apiIndex].name;
                    db.geminiApis[apiIndex].updatedAt = new Date().toISOString();

                    await fs.writeFile(DB_PATH, JSON.stringify(db, null, 2));
                    return db.geminiApis[apiIndex];
                } else {
                ...
                const newApi = {
                    id: Date.now(),
                    apiKey: apiKey,
                    name: name || `API Key ${db.geminiApis.length + 1}`,
            ...
            console.error('Error adding/updating Gemini API:', error);
            throw error;
        }
    }
